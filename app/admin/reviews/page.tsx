...
export default async function AdminReviewsPage({ searchParams }: { searchParams: Promise<{ edit?: string; saved?: string; statusUpdated?: string; deleted?: string; error?: string; q?: string; filter?: string; groupBy?: string }> }) {
  ...
  return <div className="space-y-6"><AdminModuleHeader eyebrow="Trust" title="Reviews" description="Moderate customer reviews, approve strong social proof and hide low quality submissions." />
    {params.saved ? <ActionMessage type="success" message="Review saved." /> : null}{params.statusUpdated ? <ActionMessage type="success" message="Review status updated." /> : null}{params.deleted ? <ActionMessage type="success" message="Review deleted." /> : null}{params.error === "invalid-status" ? <ActionMessage type="error" message="Invalid review status." /> : null}{params.error === "invalid-review" ? <ActionMessage type="error" message="Please check the review fields. Customer name must be present and comment must be at least 12 characters." /> : null}{params.error === "duplicate-booking-review" ? <ActionMessage type="error" message="This booking is already linked to another review. Choose a different completed booking or leave booking request empty." /> : null}
